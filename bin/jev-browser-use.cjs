...
// bin/jev-browser-use-bin and, for global installs, re-points the global `jev-browser-use`
...
function runBinary() {
  ...
  if (r.status === null) {
    ...
    console.error(`jev-browser-use: could not run the prebuilt binary ${bin}: ${why}`);
    console.error(
      "jev-browser-use: hints: check `file " + bin + "` matches this OS/CPU (glibc x64/arm64 and macOS builds only; no musl/baseline yet);\n" +
        "  if the download was truncated, delete it and rerun `jev-browser-use` (it re-downloads) or `npm rebuild -g jev-browser-use`;\n" +
...
function runDev() {
  ...
  if (r.error && r.error.code === "ENOENT") {
    console.error(`jev-browser-use: no binary at ${dl.binPath} and bun not found to run ${main}. Install bun or run \`bun run build\`.`);
    process.exit(1);
  }
  if (r.status === null) {
    console.error(`jev-browser-use: bun failed to run ${main}: ${r.error ? r.error.message : r.signal ? `killed by ${r.signal}` : "unknown error"}`);
    process.exit(1);
  }
  process.exit(r.status);
}
...
if (fs.existsSync(dl.binPath)) runBinary();
...
else if (process.env.JEV_BROWSER_USE_SKIP_DOWNLOAD) {
  console.error(`jev-browser-use: native binary missing at ${dl.binPath} and JEV_BROWSER_USE_SKIP_DOWNLOAD is set, so it will not be downloaded.`);
  ...
  if (plat.error) {
    console.error(`jev-browser-use: ${plat.error}.`);
    process.exit(1);
  }
  console.error(`jev-browser-use: downloading binary v${dl.VERSION}...`);
  dl.downloadBinary()
    .then(() => runBinary())
    .catch((err) => {
      console.error(`jev-browser-use: could not download the binary (${err.message}).`);
      console.error("jev-browser-use: the binary is fetched on first run; if you are offline, retry when connected, or:");
      console.error(dl.manualHint());
      process.exit(1);
    });
}
