# jev-browser-use benchmarks

Measures the numbers from `docs/design-decisions.md` §8 against an isolated
`JEV_BROWSER_USE_HOME` temp dir and a warm headless daemon. Everything is cleaned up at
...
- Uses `dist/jev-browser-use` when it exists (`bun run build`), else `bun run src/cli/main.ts`
...
| --- | --- | --- |
| `1+1` | wall time of one `jev-browser-use --headless -e '1+1'` process | 25 ms |
