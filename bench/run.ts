/**
 * jev-browser-use benchmark suite.
 *
 *   bun run bench/run.ts [--check] [--bin PATH|dev] [--runs N]
 *
 * Runs against an isolated JEV_BROWSER_USE_HOME and a warm headless daemon, prints a
...
/** JEV_BROWSER_USE_BENCH_SCALE multiplies every target (CI runners are slower than a dev box). */
const SCALE = Math.max(1, Number(process.env.JEV_BROWSER_USE_BENCH_SCALE ?? "1") || 1);
...
async function main(): Promise<void> {
  ...
  const home = fs.mkdtempSync(path.join(os.tmpdir(), "jev-browser-use-bench-"));
  const env: Record<string, string> = { ...(process.env as Record<string, string>), JEV_BROWSER_USE_HOME: home };
  delete env.NODE_PATH;

  const distBin = path.join(ROOT, "dist/jev-browser-use");
  ...
  const runJevBrowserUse = async (args: string[], stdin?: string): Promise<Res> => {
  ...
  const script = async (code: string): Promise<{ ms: number; value: string | undefined }> => {
    const r = await runJevBrowserUse(["--headless", "--json", "--quiet-page", "-e", code]);
  ...
  try {
    ...
    await measure(
      ...
      async () => {
        ...
        const b = await runJevBrowserUse(["--json", "browsers"]);
  ...
  console.log("");
  if (SCALE !== 1) console.log(`(targets scaled x${SCALE} via JEV_BROWSER_USE_BENCH_SCALE)`);
