 ...
 *   bun run scripts/build.ts all        -> build/daemon.js + dist/jev-browser-use
...
const outfile = outfileIdx > 0 ? process.argv[outfileIdx + 1]! : path.join(root, "dist", "jev-browser-use");
...
async function buildDaemon(): Promise<void> {
  ...
  const result = await Bun.build({
    ...
    sourcemap: "none",
    // yauzl is bundled so `jev-browser-use install` works without an `unzip` binary;
