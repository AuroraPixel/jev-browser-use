// Shared by scripts/postinstall.cjs and bin/jev-browser-use.cjs: locate, download and
// verify the prebuilt jev-browser-use binary for this platform, and (for global npm
// installs) point the global `jev-browser-use` symlink straight at it.
//
// Env knobs:
//   JEV_BROWSER_USE_SKIP_DOWNLOAD=1       never download (postinstall and shim both honour it)
//   JEV_BROWSER_USE_DOWNLOAD_BASE=URL     full URL prefix holding jev-browser-use-<os>-<arch> + SHA256SUMS
//                                (default https://github.com/<REPO>/releases/download/v<VERSION>)
//   JEV_BROWSER_USE_REPO=owner/name       GitHub repo used to build the default base
//   JEV_BROWSER_USE_VERSION=x.y.z         release version used to build the default base
"use strict";
...
const REPO = process.env.JEV_BROWSER_USE_REPO || "AuroraPixel/jev-browser-use";
const VERSION = process.env.JEV_BROWSER_USE_VERSION || pkg.version;

/** Where the binary lives inside the package. */
const binPath = path.join(pkgRoot, "bin", process.platform === "win32" ? "jev-browser-use-bin.exe" : "jev-browser-use-bin");
/** The JS shim (what npm links `jev-browser-use` to before relinkGlobal runs). */
const shimPath = path.join(pkgRoot, "bin", "jev-browser-use.cjs");
...
function downloadBase() {
  const b = process.env.JEV_BROWSER_USE_DOWNLOAD_BASE;
...
function platformAsset() {
  ...
  return { asset: `jev-browser-use-${plat}-${arch}`, plat, arch };
}
...
function fetch(url, redirects = 0) {
  return new Promise((resolve, reject) => {
    ...
    mod
      .get(url, { headers: { "user-agent": "jev-browser-use-postinstall" } }, (res) => {
...
async function downloadBinary(opts = {}) {
  ...
  const { asset } = p;
  log(`jev-browser-use: downloading ${asset} v${VERSION} from ${base}`);
  ...
  log(`jev-browser-use: installed binary at ${dest}`);
  return { dest, asset, base };
}

/**
 * For a *global* npm install only: if `<prefix>/bin/jev-browser-use` is a symlink that
 * resolves to THIS package's bin/jev-browser-use.cjs, re-point it at the binary so Node
...
function relinkGlobal(opts = {}) {
  ...
  try {
    ...
    if (opts.log) opts.log(`jev-browser-use: ${link} now points directly at the binary`);
    return true;
  } catch {
...
function manualHint() {
  const p = platformAsset();
  const asset = p.asset || "jev-browser-use-<os>-<arch>";
  const lines = [
    `  - run \`npm rebuild -g jev-browser-use\` (re-runs the download; make sure JEV_BROWSER_USE_SKIP_DOWNLOAD is unset),`,
