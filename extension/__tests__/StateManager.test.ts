...
describe("StateManager", () => {
  ...
  describe("getState", () => {
    ...
    it("should return stored state when available", async () => {
      await fakeBrowser.storage.local.set({
        jevBrowserUseActiveState: { isActive: true },
      });
  ...
  describe("setState", () => {
    it("should persist state to storage", async () => {
      ...
      const stored = await fakeBrowser.storage.local.get("jevBrowserUseActiveState");
      expect(stored.jevBrowserUseActiveState).toEqual({ isActive: true });
    });
