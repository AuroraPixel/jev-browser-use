# Driving a host-owned Chromium (bb in-app browser) with jev-browser-use

jev-browser-use can attach to any Chromium that a host application owns, without a
devtools port, through the **socket source**:

```
jev-browser-use --connect unix:/path/to/cdp.sock -e '...'
```
...
## Behavior on the jev-browser-use side

- The browser key is `socket:unix:/path`; named pages live under that key in
  `~/.jev-browser-use/v1/pages/`.
- Nothing is launched or killed; `jev-browser-use stop socket:unix:/path` only
  disconnects.
