jev-browser-use {{VERSION}} — browser automation CLI for coding agents (Puppeteer scripts, named pages, snapshot refs)

USAGE
  jev-browser-use [flags] < script.js        run a script from stdin (heredoc)
  jev-browser-use [flags] -e 'code'          run inline code
  jev-browser-use [flags] run FILE           run a script file
  jev-browser-use pages | browsers | status | stop [NAME] | install [--force] | install-skill [--claude|--codex|--agents]
  jev-browser-use migrate-from-doobie       copy durable ~/.doobie state into the isolated v1 state directory
  jev-browser-use chrome [--profile NAME] [--port N] [--chrome PATH] [--headless] [--list] [URL]
  jev-browser-use relay [--port 9222]       bridge the jev-browser-use Chrome extension to this CLI/MCP
  jev-browser-use help [topic]               topics: quickstart workflow scripts pages snapshot refs screenshots waiting forms
                                    errors output connect extension chrome config migration json mcp jev examples tips
FLAGS
  -b, --browser NAME      launch/reuse a persistent named profile (default: "default"); ~/.jev-browser-use/v1/browsers/NAME
  ...
  Flags accept --flag=value, before or after the subcommand, except: everything after install/install-skill/chrome/relay
...
## quickstart
  ...
  EOF
  jev-browser-use -e 'const p = await browser.getPage("main"); await p.click("ref/e6"); p.url()'        # act by ref
  jev-browser-use -e 'const p = await browser.getPage("main"); (await p.shot()).path'                      # then Read the image
  First run starts a daemon + Chrome (~0.5 s); later runs reuse both (~15 ms). No Chrome? `jev-browser-use install`.
## workflow
  ...
  Inside page.evaluate(...) write plain browser JavaScript (no TypeScript, no jev-browser-use globals, no closures over script
...
## scripts
  ...
  Globals: browser, console, saveFile(name, data) -> path, readFile(name) -> string (both jailed to ~/.jev-browser-use/v1/tmp,
...
## pages
  browser.getPage(name)       get-or-create a named tab; the name -> tab mapping is stored in ~/.jev-browser-use/v1/pages/ and
                              survives daemon restarts while Chrome lives. A closed tab is recreated on next use.
  browser.getPage(targetId)   attach to any open tab by its 32-hex CDP target id (from listPages / `jev-browser-use pages`).
  ...
  `jev-browser-use browsers` lists them; `jev-browser-use pages` lists tabs for every running browser (with -b/--connect it launches/
  attaches that browser and lists only its tabs); `jev-browser-use stop NAME` closes one (profile persists; exit 1 if none
  matched; a script running on it fails with BrowserStoppedError); `jev-browser-use stop` closes all and exits the daemon. Tabs
  ...
  Puppeteer itself (page.browser().newPage(), popups) get the jev-browser-use helpers too; with --connect only tabs you touch
  (getPage, newPage, their popups) are extended — the user's other tabs keep their dialogs and scripts. Downloads from
  launched browsers land in ~/.jev-browser-use/v1/tmp/downloads/<name> (readFile("downloads/<name>") reads them); attached browsers
...
## screenshots
  ...
  Writes ~/.jev-browser-use/v1/tmp/shot-<ts>-<n>.jpg (JPEG q80) and prints `[image] PATH (WxH)`; open it with your image/Read tool.
...
## errors
  ...
  Chrome's error page). Recover by reconnecting to the same page name and checking state. `jev-browser-use stop` during a script
  ...
  errors (exit 1): `TimeoutError: Waiting for selector \`#x\` failed (cause: Waiting failed: 5000ms exceeded)`; an error's cause is appended as ` (cause: ...)`; e.name / e.constructor.name are the real Puppeteer names.
  ...
  "No Chrome found": `jev-browser-use install` or JEV_BROWSER_USE_CHROME. Daemon trouble: `jev-browser-use status` (log tail), `jev-browser-use stop`, retry.
  "profile ... is in use by another Chrome (pid N)": a hand-started Chrome holds that profile dir; stop it or use another
  -b NAME. Snap Chromium (Ubuntu /snap/bin) cannot read ~/.jev-browser-use/v1: use `jev-browser-use install` or JEV_BROWSER_USE_CHROME=<deb/CfT>.

## output
  stdout: console output and `[image] PATH (WxH)` lines (one per page.shot()) streamed live; the return value last. stderr: console.warn/error, the error block, the page console block. stderr is never capped.
  Cap: after 50k chars of stdout the rest is spilled to ~/.jev-browser-use/v1/tmp/out-<id>.txt and at the end jev-browser-use prints
...
## connect
  --connect with no value = auto: probes ports remembered by `jev-browser-use chrome`, then 9222-9229 on 127.0.0.1 (400 ms each).
  ...
  Attached browsers are never closed by --idle-timeout; `jev-browser-use stop <key>` only disconnects. Named pages work the same
  (stored per endpoint; ws URL query/credentials are stripped from keys and logs); see the user's existing tabs with browser.listPages(), then getPage(targetId). Only tabs you touch are extended (see `help pages`).
## extension
  Run `jev-browser-use relay` (loopback 9222) and enable the bundled Chrome extension; its popup should show Connected.
  CLI: `--connect http://127.0.0.1:9222`; MCP: `mcp --connect http://127.0.0.1:9222 -t 60`. Keep this URL for Jev too.
  Uses existing profile/logins; exposes only tabs created by the extension. One CDP client; share JEV_BROWSER_USE_HOME/daemon.
  Health: GET http://127.0.0.1:9222/. No browser-wide settings/contexts, Browser.close or extra page CDP sessions; see docs/jev.md.
## chrome
  jev-browser-use chrome [--profile NAME] [--port N] [--chrome PATH] [--headless] [URL]   launch your real installed Chrome as
  a normal OS process with --remote-debugging-port on a dedicated profile (~/.jev-browser-use/v1/chrome-profiles/NAME, default NAME
  "chrome") and remember the port in ~/.jev-browser-use/v1/chrome-ports.json so a bare `jev-browser-use --connect` finds it. This is the path
  for Google sign-in and other logins that reject automation-launched Chrome: sign in by hand once, then automate.
  It prefers a system Chrome (then JEV_BROWSER_USE_CHROME/config, then `jev-browser-use install`'s Chrome for Testing, then Playwright's
  ...
  (~/.jev-browser-use/v1/chrome-logs/NAME.log) are printed, exit 1, nothing recorded. On Linux a sandbox failure is retried once
  with --no-sandbox (remembered in launch-state.json). --headless passes --headless=new (CI/tests).
  `jev-browser-use chrome --list` prints detected Chromes. (Chrome 136+ ignores the port flag on its default profile, hence the dedicated --user-data-dir.) A --connect that finds nothing prints this launch hint.

## config
  ~/.jev-browser-use/v1/config.json: { "headless": false, "idleTimeout": "30m", "chrome": "/path/to/chrome", "timeout": 30, "ignoreHttpsErrors": false }
  Precedence everywhere: flag > env > config.json > default. Env: JEV_BROWSER_USE_HOME (state dir, default ~/.jev-browser-use/v1:
       daemon.sock/.pid/.log, config.json, tmp/ (shots, out-*.txt, downloads/), browsers/NAME/profile[-headless]
       [-insecure], chrome-profiles/NAME, chrome-logs/, pages/, chrome/), JEV_BROWSER_USE_CHROME (executable), JEV_BROWSER_USE_IDLE_TIMEOUT
       (legacy JEV_BROWSER_USE_IDLE_TIMEOUT_MS is also accepted),
       JEV_BROWSER_USE_SOCKET. Durations: 30s, 5m, 1h, 0 (off), or milliseconds.
  Chrome lookup: JEV_BROWSER_USE_CHROME > config.chrome > ~/.jev-browser-use/v1/chrome (from `jev-browser-use install`, Chrome for Testing via
  @puppeteer/browsers; no unzip needed, ~150 MB) > system Chrome/Chromium/Edge/Brave > Playwright's cached Chromium.
  `jev-browser-use install-skill [--claude|--codex|--agents]` writes SKILL.md to ~/.claude|.codex|.agents/skills/jev-browser-use (default all).

## migration
  jev-browser-use uses ~/.jev-browser-use/v1, independent of dev-browser and doobie runtime state.
  To carry state forward from the predecessor implementation, first run `doobie stop`, then:
    jev-browser-use migrate-from-doobie
  This copies config, browser profiles, named-page maps, downloaded Chrome, saved files, and launch preferences from
  ~/.doobie (or DOOBIE_HOME) into JEV_BROWSER_USE_HOME. It never modifies the source and refuses to overwrite existing v1
  state. Older upstream Playwright profiles are not compatible; this command only imports doobie state.
...
## json
  ...
  The process exit code equals done.exitCode. Output is never capped in --json mode.  jq: jev-browser-use --json -e '...' | jq -c 'select(.type=="result").data'

## mcp
  jev-browser-use mcp [-b NAME] [--headless] [--connect URL] [-t S]    Model Context Protocol server over stdio (JSON-RPC 2.0).
  Tools: jev_browser_use_run { script, browser?, headless?, connect?, timeout? } (console output, return value, errors, page.shot()
...
## examples
  # Inspect a page you have never seen (interactive elements only); first tracked call prints the full tree jev-browser-use <<'EOF' const page = await browser.getPage("shop");
  await page.goto("https://shop.example.com", { timeout: 15000 });
  await page.waitForLoad();
  (await page.snapshot({ interactive: true, track: "shop" })).full
  EOF
  # Act by ref and verify with the incremental diff (same track name, same options) jev-browser-use <<'EOF' const page = await browser.getPage("shop");
  ...
  snap.incremental
  EOF
  # Log in, persist the session (profile "work" keeps cookies across runs) jev-browser-use -b work <<'EOF' const page = await browser.getPage("login");
  ...
  EOF
  # Extract structured data with plain JS in the page (returned arrays/objects print as JSON) jev-browser-use --headless -e 'const p = await browser.getPage("hn"); await p.goto("https://news.ycombinator.com");
  ...
  jev-browser-use -e 'const p = await browser.getPage("shop"); await p.shot({ name: "shop.jpg" })' jev-browser-use -e 'const p = await browser.getPage("shop"); await p.mouse.click(412, 233); await p.waitForLoad(); p.url()'
  # Your own Chrome (after `jev-browser-use chrome`): list tabs, attach one by target id, scoped snapshot with boxes
  jev-browser-use --connect -e 'await browser.listPages()' jev-browser-use --connect -e 'const p = await browser.getPage("4F0C...32HEX"); await p.snapshot({ scope: "e42", boxes: true })'
  # Test a local dev app: console errors surface automatically on stderr as [page:dev] lines jev-browser-use -t 20 <<'EOF' const page = await browser.getPage("dev");
  ...
  jev-browser-use --ignore-https-errors -e 'const p = await browser.getPage("dev"); await p.goto("https://localhost:8443"); p.url()'
  jev-browser-use -e 'const p = await browser.getPage("shop"); ({ url: p.url(), title: await p.title(), shot: (await p.shot()).path })'
  jev-browser-use -e 'const p = await browser.getPage("embed"); await p.click("ref/f1e3"); (await p.ref("f1e3")).evaluate(e => e.textContent)'
