/**
 * Run the jev-browser-use CLI (dev mode) against an isolated JEV_BROWSER_USE_HOME.
 */
...
export function makeCliEnv(prefix = "jev-browser-use-test-"): CliEnv {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), prefix));
  const baseEnv: Record<string, string> = { ...(process.env as Record<string, string>), JEV_BROWSER_USE_HOME: home };
