...
import { extendPage, type JevBrowserUsePage } from "../../src/page/extend.ts";
...
export async function getBrowser(): Promise<Browser> {
  ...
  if (!chrome) throw new Error("no Chrome found for tests; set JEV_BROWSER_USE_CHROME");
...
export async function newPage(): Promise<JevBrowserUsePage> {
...
export async function withPage<T>(fn: (page: JevBrowserUsePage) => Promise<T>): Promise<T> {
...
export type { Page, JevBrowserUsePage };
