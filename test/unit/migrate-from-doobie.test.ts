...
const previousTarget = process.env.JEV_BROWSER_USE_HOME;

afterEach(() => {
  for (const root of roots.splice(0)) fs.rmSync(root, { recursive: true, force: true });
  if (previousSource === undefined) delete process.env.DOOBIE_HOME;
  else process.env.DOOBIE_HOME = previousSource;
  if (previousTarget === undefined) delete process.env.JEV_BROWSER_USE_HOME;
  else process.env.JEV_BROWSER_USE_HOME = previousTarget;
});

function setup(): { source: string; target: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "jev-browser-use-migrate-"));
  roots.push(root);
  const source = path.join(root, "doobie");
  const target = path.join(root, "jev-browser-use-v1");
  ...
  process.env.JEV_BROWSER_USE_HOME = target;
