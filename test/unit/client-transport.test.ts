...
describe("client transport: Bun.connect backpressure", () => {
  test("a 2 MB payload arrives intact when the server reads slowly", async () => {
    const dir = tmpDir("jev-browser-use-conn-");
...
describe("profile lock safety predicate (classifyLockHolder)", () => {
  const dir = "/home/u/.jev-browser-use/browsers/work/profile";
  ...
  test("chromePortPids reads pids recorded by `jev-browser-use chrome`", () => {
    const home = tmpDir("jev-browser-use-home-");
    const prev = process.env.JEV_BROWSER_USE_HOME;
    process.env.JEV_BROWSER_USE_HOME = home;
    try {
      ...
      if (prev === undefined) delete process.env.JEV_BROWSER_USE_HOME;
      else process.env.JEV_BROWSER_USE_HOME = prev;
    }
  });
});

describe("profile dirs", () => {
  test("headed and headless instances of one name use different user-data-dirs; jev-browser-use chrome has its own root", () => {
    const home = tmpDir("jev-browser-use-home-");
    const prev = process.env.JEV_BROWSER_USE_HOME;
    process.env.JEV_BROWSER_USE_HOME = home;
    try {
...
describe("Chrome for Testing discovery (installedCandidates)", () => {
  ...
  test("finds the linux and the (deep) macOS layouts under a temp JEV_BROWSER_USE_HOME", () => {
    const home = tmpDir("jev-browser-use-home-");
    const prev = process.env.JEV_BROWSER_USE_HOME;
    process.env.JEV_BROWSER_USE_HOME = home;
    try {
...
describe("jev-browser-use chrome picks the user's real browser", () => {
...
describe("read-only JEV_BROWSER_USE_HOME", () => {
  test("ensureHome fails fast with a clear message", () => {
    ...
    const base = tmpDir("jev-browser-use-ro-");
    const home = path.join(base, "home");
    fs.mkdirSync(home, { mode: 0o500 });
    const prev = process.env.JEV_BROWSER_USE_HOME;
    process.env.JEV_BROWSER_USE_HOME = home;
    try {
      expect(() => ensureHome()).toThrow(/cannot write to JEV_BROWSER_USE_HOME .*EACCES/);
    } finally {
