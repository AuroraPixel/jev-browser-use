...
import { jailPath, sanitizeName, sanitizeKey, paths, jevBrowserUseHome } from "../../src/shared/paths.ts";
import { LineDecoder, encodeFrame } from "../../src/shared/protocol.ts";

test("default home is versioned away from the v0.2 daemon", () => {
  const previous = process.env.JEV_BROWSER_USE_HOME;
  delete process.env.JEV_BROWSER_USE_HOME;
  try {
    expect(jevBrowserUseHome()).toBe(path.join(process.env.HOME!, ".jev-browser-use", "v1"));
  } finally {
    if (previous === undefined) delete process.env.JEV_BROWSER_USE_HOME;
    else process.env.JEV_BROWSER_USE_HOME = previous;
  }
});
...
describe("resolveIdleTimeoutMs precedence", () => {
  const prev = process.env.JEV_BROWSER_USE_IDLE_TIMEOUT;
  const prevLegacy = process.env.JEV_BROWSER_USE_IDLE_TIMEOUT_MS;
  afterEach(() => {
    if (prev === undefined) delete process.env.JEV_BROWSER_USE_IDLE_TIMEOUT;
    else process.env.JEV_BROWSER_USE_IDLE_TIMEOUT = prev;
    if (prevLegacy === undefined) delete process.env.JEV_BROWSER_USE_IDLE_TIMEOUT_MS;
    else process.env.JEV_BROWSER_USE_IDLE_TIMEOUT_MS = prevLegacy;
  });
  test("flag > env > config > default", () => {
    delete process.env.JEV_BROWSER_USE_IDLE_TIMEOUT;
    ...
    process.env.JEV_BROWSER_USE_IDLE_TIMEOUT = "3m";
    ...
    process.env.JEV_BROWSER_USE_IDLE_TIMEOUT = "";
    expect(resolveIdleTimeoutMs(undefined, { idleTimeout: "2m" })).toBe(120_000);
  });
  test("bad flag throws", () => {
    expect(() => resolveIdleTimeoutMs("soon", {})).toThrow();
  });
  test("accepts the 0.2.x JEV_BROWSER_USE_IDLE_TIMEOUT_MS name as a fallback", () => {
    delete process.env.JEV_BROWSER_USE_IDLE_TIMEOUT;
    process.env.JEV_BROWSER_USE_IDLE_TIMEOUT_MS = "45s";
...
describe("sanitizeName / sanitizeKey", () => {
  ...
  test("pagesFile uses the sanitized key", () => {
    expect(paths.pagesFile("work:headless")).toBe(path.join(jevBrowserUseHome(), "pages", "work__headless.json"));
  });
});
