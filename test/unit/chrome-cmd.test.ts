/**
 * `jev-browser-use chrome` launch verification: a Chrome that dies right away is an
...
async function chrome(args: string[], env: Record<string, string> = {}): Promise<{ code: number; stdout: string; stderr: string }> {
  ...
  const proc = Bun.spawn([process.execPath, runner, ...args], {
    cwd: ROOT,
    env: { ...process.env, JEV_BROWSER_USE_HOME: home, NODE_PATH: "", ...env },
    stdout: "pipe",
    stderr: "pipe",
  });
...
beforeAll(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), "jev-browser-use-chrome-cmd-"));
  prevHome = process.env.JEV_BROWSER_USE_HOME;
  process.env.JEV_BROWSER_USE_HOME = home;
  cli = makeCliEnv("jev-browser-use-chrome-cli-");
});
afterAll(async () => {
  if (prevHome === undefined) delete process.env.JEV_BROWSER_USE_HOME;
  else process.env.JEV_BROWSER_USE_HOME = prevHome;
...
describe("jev-browser-use chrome verifies the launch", () => {
...
describe("jev-browser-use help <unknown topic>", () => {
  test("exits 2 with the message on stderr; known topics still exit 0", async () => {
    const r = await cli.run(["help", "bogus-topic"]);
    expect(r.code).toBe(2);
    expect(r.stdout).toBe("");
    expect(r.stderr).toMatch(/^jev-browser-use: No help topic "bogus-topic"\. Topics: .*quickstart/);
