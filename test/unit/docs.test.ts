/**
 * Docs consistency: docs/help.md (embedded in `jev-browser-use --help`) must stay within
...
const SKILL = fs.readFileSync(path.join(ROOT, "skills/jev-browser-use/SKILL.md"), "utf8");
const DESIGN = fs.readFileSync(path.join(ROOT, "docs/design-decisions.md"), "utf8");

const cli = makeCliEnv("jev-browser-use-docs-");
afterAll(async () => {
  await cli.cleanup();
});
...
test("help.md stays within the line budget and every topic is present", () => {
  ...
  expect(HELP).toContain("jev-browser-use chrome [--profile NAME] [--port N] [--chrome PATH] [--headless] [--list] [URL]");
...
test("README, SKILL and design doc reflect the current behaviour", () => {
  ...
  expect(README).toContain("JEV_BROWSER_USE_DOWNLOAD_BASE");
  expect(README).toContain("## Releasing");
  expect(README).toContain("bring-to-front lock");
  expect(SKILL).toContain("bun install --frozen-lockfile && bun run build");
