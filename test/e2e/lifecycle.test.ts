...
beforeAll(async () => {
  cli = makeCliEnv("jev-browser-use-e2e-life-");
...
describe("usage, version, help (no daemon needed)", () => {
  test("unknown flag -> exit 2 with a jev-browser-use: message", async () => {
    const r = await cli.run(["--bogus"]);
    expect(r.code).toBe(2);
    expect(r.stdout).toBe("");
    expect(r.stderr).toBe("jev-browser-use: unknown flag --bogus\n");
  });
  ...
  test("--version", async () => {
    ...
    expect(r).toEqual({ code: 0, stdout: `jev-browser-use ${VERSION}\n`, stderr: "" });
  });
  ...
  test("install-skill writes SKILL.md under HOME", async () => {
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), "jev-browser-use-home-"));
    try {
      const r = await cli.run(["install-skill", "--claude"], { env: { HOME: fakeHome } });
      expect(r.code).toBe(0);
      const file = path.join(fakeHome, ".claude", "skills", "jev-browser-use", "SKILL.md");
      ...
      expect(r2.code).toBe(0);
      expect(fs.existsSync(path.join(fakeHome, ".codex", "skills", "jev-browser-use", "SKILL.md"))).toBe(true);
      expect(fs.existsSync(path.join(fakeHome, ".agents", "skills", "jev-browser-use", "SKILL.md"))).toBe(true);
      // unknown target
...
describe("--connect to an existing Chrome", () => {
  ...
  beforeAll(async () => {
    ...
    userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "jev-browser-use-ext-chrome-"));
  ...
  test("--connect http://127.0.0.1:PORT reaches the same browser and the same named page", async () => {
    ...
    const b = await cli.run(["browsers"]);
    expect(b.stdout.trim().split("\n").length).toBe(1);
    // the external Chrome sees exactly one extra tab named by jev-browser-use (plus its own initial + /test page)
  ...
  test("jev-browser-use pages --connect lists the external Chrome's tabs", async () => {
