 ...
 * tracker) while jev-browser-use's pages, their popups, and tabs reached via
...
beforeAll(async () => {
  cli = makeCliEnv("jev-browser-use-e2e-life3-");
...
/**
 * From the user's own client: run confirm() in a tab. If jev-browser-use's dialog
...
describe("--connect: the user's own tabs are not extended", () => {
  ...
  beforeAll(async () => {
    ...
    port = await freePort();
    userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "jev-browser-use-ext3-chrome-"));
  ...
  test("a tab the user opens while jev-browser-use is attached keeps its dialogs and gets no load tracker", async () => {
    ...
    expect(await userPage.evaluate(() => typeof (window as unknown as { __jevBrowserUseLoad?: unknown }).__jevBrowserUseLoad)).toBe("undefined");
    // and jev-browser-use did not report it as one of its pages in that run's stderr (nothing ran)
  ...
  test("jev-browser-use's own named page auto-dismisses dialogs (extended)", async () => {
  ...
  test("a popup opened from a jev-browser-use page is extended (opener is a touched page)", async () => {
...
r`,
  ...
  test("a user tab reached via getPage(targetId) is extended lazily, on first access", async () => {
    ...
    // and it stays extended (jev-browser-use touched it): the user's confirm is now auto-dismissed
...
describe("pages/status do not reset --idle-timeout", () => {
  ...
  test("`jev-browser-use pages -b NAME` and `status` keep the previously chosen idle timeout; a run with a new one changes it", async () => {
  ...
  test("`jev-browser-use pages -b NEW` on a not-yet-running browser launches it with the default idle timeout", async () => {
