...
beforeAll(async () => {
  cli = makeCliEnv("jev-browser-use-e2e-life2-");
  srv = await startServer({
    ...
    "/dl": () =>
      new Response("hello download", {
        headers: { "content-type": "application/octet-stream", "content-disposition": 'attachment; filename="jev-browser-use-dl.txt"' },
      }),
  });
});
...
describe("large requests (socket backpressure)", () => {
  ...
  test("a 1 MB script through the compiled binary", async () => {
    ...
    const outfile = path.join(cli.home, "jev-browser-use-bin");
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "jev-browser-use-e2e-bin-"));
    const runBin = async (args: string[], stdin?: string) => {
      const proc = Bun.spawn([outfile, ...args], {
        env: { ...process.env, JEV_BROWSER_USE_HOME: home, NODE_PATH: "" },
  ...
  test("yauzl is bundled into the daemon (jev-browser-use install works without unzip)", () => {
...
describe("restored tabs and downloads", () => {
  ...
  test("downloads land in JEV_BROWSER_USE_HOME/tmp/downloads", async () => {
    const dlDir = path.join(cli.home, "tmp", "downloads");
    const r = await cli.run([...H, "-b", "dl"], {
      stdin: `const p = await browser.getPage("d"); await p.goto(${JSON.stringify(srv.url("/"))}); await p.evaluate((u) => { const a = document.createElement('a'); a.href = u; a.download = 'jev-browser-use-dl.txt'; document.body.appendChild(a); a.click(); }, ${JSON.stringify(srv.url("/dl"))}); await new Promise(r => setTimeout(r, 500)); 'ok'`,
    });
    expect(r.code).toBe(0);
    const ok = await waitFor(() => fs.existsSync(dlDir) && fs.readdirSync(dlDir).some((f) => f.startsWith("jev-browser-use-dl") && !f.endsWith(".crdownload")), 10_000);
    expect(ok).toBe(true);
    const f = fs.readdirSync(dlDir).find((f) => f.startsWith("jev-browser-use-dl"))!;
...
describe("--ignore-https-errors", () => {
  test("self-signed server fails without the flag and works with it (separate instance)", async () => {
    ...
    if (!openssl) return;
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "jev-browser-use-tls-"));
...
describe("broken stdout pipe", () => {
  test("`jev-browser-use -e big | head -c 100` prints no EPIPE stack", async () => {
    ...
    const proc = Bun.spawn(["bash", "-c", script], {
      cwd: ROOT,
      env: { ...process.env, JEV_BROWSER_USE_HOME: cli.home, NODE_PATH: "" },
      stdout: "pipe",
      stderr: "pipe",
    });
