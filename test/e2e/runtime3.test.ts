 ...
 * cross-realm error logging, failed-goto page line, jev-browser-use stop under a run.
 */
...
beforeAll(async () => {
  cli = makeCliEnv("jev-browser-use-e2e-rt3-");
...
await p.goto("http://127.0.0.1:1/")`;
  ...
  test("jev-browser-use stop during a run surfaces as BrowserStoppedError", async () => {
...
describe("relative file paths resolve against the caller's cwd", () => {
  test("uploadFile, page.screenshot({path}), handle.screenshot({path}) and pdf({path})", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "jev-browser-use-cwd-"));
