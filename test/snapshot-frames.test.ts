...
import { withPage, closeBrowser, type JevBrowserUsePage } from "./helpers/browser.ts";
...
async function sameFrame(page: JevBrowserUsePage) {
...
async function navigateChild(page: JevBrowserUsePage, url: string) {
...
async function expectOldRefRejected(page: JevBrowserUsePage, ref: string): Promise<void> {
...
test("frame refs survive same-document navigation but never alias after reload, renderer swap, parent reload, or detach", async () => {
  await withPage(async (page) => {
    ...
    // CDP acknowledgement can precede the cross-process renderer's DOM update.
    // Observe the effect; never retry the click to make this assertion pass.
    await frame.waitForFunction(() => document.querySelector('#cross')?.textContent === 'Cross clicked', { timeout: 2000 });
...
test("delayed frame ref queries and actions cannot cross a document boundary", async () => {
  await withPage(async (page) => {
    ...
    // Attach a rejection handler before allowing navigation to race the call.
    // The host guard and the in-page ref guard can reject at different points.
    const delayedRef = page.ref(first).then(() => null, error => error as Error);
    await atQuery;
    await frame.goto(frame.url(), { waitUntil: "domcontentloaded" });
    const second = childRef((await page.snapshot()) as string, "Same child");
    releaseQuery();
    expect(await delayedRef).toBeInstanceOf(Error);
    expect((await delayedRef)?.message).toMatch(/stale or unknown|Frame f\d+ .* (gone|navigated)/);
    frame.$ = originalQuery;
    ...
    const delayedClick = page.click(`ref/${second}`).then(() => null, error => error as Error);
    await atClick;
    await frame.goto(frame.url(), { waitUntil: "domcontentloaded" });
    const third = childRef((await page.snapshot()) as string, "Same child");
    releaseClick();
    expect(await delayedClick).toBeInstanceOf(Error);
    expect((await delayedClick)?.message).toMatch(/stale or unknown|Frame f\d+ .* (gone|navigated)/);
...
test("a delayed snapshot stays bound to its captured execution context", async () => {
  await withPage(async (page) => {
    ...
    oldDocument.evaluate = async function (expression: string) {
      if (!paused && expression.includes("window.__jevBrowserUse.snapshot")) {
        paused = true;
        entered();
        await gate;
      }
    ...
    }).isolatedRealm().evaluate("window.__jevBrowserUseRefState.frameKey");
