...
async function framesReady(page: JevBrowserUsePage, n: number) {
...
test("reinstalling the in-page script never reuses ref ids already handed out", async () => {
  await withPage(async (page) => {
    ...
    await realm.evaluate(() => {
      delete (window as unknown as { __jevBrowserUse?: unknown }).__jevBrowserUse;
    });
    ...
    const v = await realm.evaluate(() => (window as unknown as { __jevBrowserUse: { version: number } }).__jevBrowserUse.version);
