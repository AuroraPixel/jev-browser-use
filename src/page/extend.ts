/**
 * Attach jev-browser-use helpers to a Puppeteer Page. Idempotent per Page instance.
 *
...
import { jev } from "../jev/controller.ts";
import { assertJevAvailable } from "../jev/lock.ts";
import { installRendering, withRendering } from "./rendering.ts";
  ...
  for (const proto of protos) {
    for (const name of gated) {
      ...
      const wrapped = function (this: unknown, ...args: unknown[]) {
        ...
        if (page) { try { assertJevAvailable(page); } catch (e) { return Promise.reject(e); } }
...
export function extendPage(page: Page): JevBrowserUsePage {
  if (EXTENDED.has(page)) return page as JevBrowserUsePage;
  EXTENDED.add(page);
  registerRefQueryHandler();
  patchPrototypes();
  ...
  const p = page as JevBrowserUsePage & Record<string, unknown>;
  ...
  p.goto = (url: string, options?: GoToOptions): Promise<HTTPResponse | null> =>
    Promise.resolve().then(() => { assertJevAvailable(page); return withRendering(page, () => origGoto(url, { waitUntil: "domcontentloaded", ...(options ?? {}) })); });
  for (const name of ["reload", "goBack", "goForward"] as const) {
    const original = page[name].bind(page);
    p[name] = (options?: Parameters<typeof original>[0]) => {
      assertJevAvailable(page);
      return withRendering(page, () => original(options));
    };
  }
  ...
  // custom query handler (it runs in the isolated realm where window.__jevBrowserUse lives);
  ...
  return page as JevBrowserUsePage;
}
