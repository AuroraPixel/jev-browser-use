 ...
 * ref map (window.__jevBrowserUse) — invisible to page scripts. extend.ts routes a
...
export function registerRefQueryHandler(): void {
  ...
  Puppeteer.registerCustomQueryHandler("ref", {
    queryOne: (node: Node, selector: string) => {
      ...
      const win = doc?.defaultView as (Window & { __jevBrowserUse?: { ref(id: string): Element | null } }) | null;
      const api = win?.__jevBrowserUse;
      if (!api) return null;
