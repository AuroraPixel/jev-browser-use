 ...
 * also keeps `window.__jevBrowserUse` invisible to page scripts.
 */
...
async function evaluateSnapshot(document: DocumentContext, opts: InPageOptions): Promise<InPageResult> {
  const expr = `${INPAGE_SCRIPT};\nwindow.__jevBrowserUse.snapshot(${JSON.stringify(opts)})`;
...
async function snapshotFrame(
  ...
  for (const info of res.iframes) {
    ...
    let suffix = "";
    try {
      const js = `window.__jevBrowserUse ? window.__jevBrowserUse.ref(${JSON.stringify(local)}) : null`;
