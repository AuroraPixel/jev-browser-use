 ...
 * tracker on window.__jevBrowserUseLoad (PerformanceObserver('resource') for request
...
function pollInPage(staleMs: number): TrackerState {
  ...
  const w = window as unknown as { __jevBrowserUseLoad?: Tracker };
  let t = w.__jevBrowserUseLoad;
  if (!t) {
    t = { lastRequestStart: -1, lastMutation: -1, inFlight: new Map(), nextId: 1, installedAt: performance.now() };
    w.__jevBrowserUseLoad = t;
    const tr = t;
