/**
 * jev-browser-use install [--force]
 * Download Chrome for Testing into ~/.jev-browser-use/v1/chrome via @puppeteer/browsers.
...
export async function installChrome(args: string[]): Promise<number> {
  ...
  if (!platform) {
    process.stderr.write("jev-browser-use install: unsupported platform\n");
    return EXIT_ERROR;
  }
  ...
  try {
    ...
    process.stderr.write(`jev-browser-use install: could not resolve the latest Chrome for Testing build: ${(err as Error).message}\n`);
    return EXIT_ERROR;
  }
  ...
  if (fs.existsSync(installDir)) {
    if (!force) {
      process.stderr.write(
        `jev-browser-use install: ${installDir} already exists but no usable Chrome was found in it.\n` +
          "Run `jev-browser-use install --force` to replace it.\n",
      );
      return EXIT_ERROR;
    }
  ...
  try {
    ...
    process.stderr.write(`\njev-browser-use install: failed: ${msg.split("\n")[0]}\n`);
