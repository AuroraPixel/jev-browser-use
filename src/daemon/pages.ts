 ...
 *
 * Names live in ~/.jev-browser-use/v1/pages/<browserKey>.json as { name: targetId } so
...
import { extendPage, type JevBrowserUsePage } from "../page/extend.ts";
...
export class PageRegistry {
  ...
  /**
   * Target ids of pages jev-browser-use has extended (named pages, getPage(targetId),
   * newPage, popups of touched pages). On attached browsers (--connect) only
   * these carry jev-browser-use's dialog/load hooks; the user's own tabs are left alone.
   */
  ...
  /** Extend a page with the jev-browser-use helpers and record it as touched. */
  adopt(page: Page): JevBrowserUsePage {
    this.touched.add(targetIdOf(page));
    return extendPage(page);
  }

  /** True when jev-browser-use has extended the page with this target id. */
  ...
  /** Name an existing page (used by tests and future `jev-browser-use pages name` command). */
