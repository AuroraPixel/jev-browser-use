...
export interface BrowserEntry {
  ...
  activeRuns: number;
  wsEndpoint?: string;
  /** True when jev-browser-use launched the process (eligible for idle cleanup). */
  launched: boolean;
}
...
export class BrowserManager {
  ...
  /** Pids of Chrome processes this daemon owns or knows to be the user's (`jev-browser-use chrome`). Never orphan candidates. */
  ...
  private async connect(
        ...
        if (outcome !== "free") {
          throw new ProfileBusyError(
            `profile "${spec.name}" (${spec.headless ? "headless" : "headed"}) is in use by another Chrome (${outcome}). ` +
              `Stop it first (\`jev-browser-use stop ${spec.name}\`, or quit that Chrome), or use a different -b NAME.`,
          );
        }
        r = await launchBrowser(spec, this.log, { timeoutMs: opts.timeoutMs });
      }
    ...
    // the jev-browser-use helpers too. Target.page() is cached per target, so extending
    // it here covers every later path to that Page.
    //
    // Launched browsers are jev-browser-use's own: every tab is extended eagerly. An
    // attached browser (--connect) belongs to the user: materializing a Page
    // for every tab they open would install jev-browser-use's dialog auto-dismiss and
    // load tracker in tabs no script ever touched. There, only popups whose
    // opener is a page jev-browser-use touched are extended eagerly; everything else
 ...
 * (after `jev-browser-use stop`, a crash or a SIGKILLed daemon). Those tabs are
...
/**
 * Launched browsers download into ~/.jev-browser-use/v1/tmp/downloads instead of the
...
/** Pids recorded by `jev-browser-use chrome` (the user's signed-in Chromes). */
...
export type LockHolder =
  ...
  /** Live Chrome owned by this daemon or by `jev-browser-use chrome`: never touch. */
 ...
 * pids that must never be killed (live BrowserManager entries, `jev-browser-use
      ...
      case "unrelated":
        if (Date.now() >= deadline) {
          return holder.kind === "ours"
            ? `pid ${holder.pid}, owned by this daemon or \`jev-browser-use chrome\``
            : holder.kind === "foreign"
