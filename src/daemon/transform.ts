...
export function transformScript(src: string): TransformResult {
  ...
  if (objectLiteral) {
    ...
    // unless it really is "use strict" (e.g. `jev-browser-use -e '"hello"'` prints hello).
  ...
  for (const stmt of program.body) {
    if (stmt.type === "ImportDeclaration" || stmt.type.startsWith("Export")) {
      throw new ScriptSyntaxError(
        "import/export are not available in jev-browser-use scripts. Everything you need is already in scope (browser, console, saveFile, readFile).",
        stmt.loc?.start.line,
      );
    }
  }
