...
export class RunGate {
  ...
  private wrap(target: object, prop: PropertyKey, fn: AnyFn, label: string, self: () => object): AnyFn {
    ...
    const wrapper = function (this: unknown, ...args: unknown[]): unknown {
      ...
      if (isPage(target) && prop !== "jev") assertJevAvailable(target);
...
export async function runScript(req: RunRequest, ctx: RunContext): Promise<RunOutcome> {
  ...
  void (async () => {
    ...
    const context = vm.createContext(sandbox, { name: "jev-browser-use-script" });
    ...
    const value = await Promise.race([withRun({ id: req.id, emit, gate, cwd: req.cwd, signal: gate.abort.signal, jev: req.jev }, () => fn()), deadlinePromise]);
...
const BROWSER_GONE_RE = /Target closed|Session closed|Connection closed|Browser (was )?disconnected|Execution context was destroyed|detached Frame|Navigating frame was detached|Protocol error.*(closed|disconnected)/i;
...
async function buildErrorFrame(
  ...
  // `jev-browser-use stop` (or a crashed Chrome) under a running script: name the cause instead of Puppeteer's
