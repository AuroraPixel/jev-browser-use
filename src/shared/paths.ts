/**
 * Filesystem layout under the jev-browser-use home directory.
 *
 *   ~/.jev-browser-use/v1/
 ...
 *     chrome-profiles/<name>             user-data-dirs for `jev-browser-use chrome --profile NAME`
 *     tmp/downloads/     download directory for launched browsers
 *     pages/<key>.json   name -> targetId map per browser key
 *     chrome/            Chrome for Testing installed by `jev-browser-use install`
 *     chrome-ports.json  ports remembered by `jev-browser-use chrome`
 *
 * JEV_BROWSER_USE_HOME overrides the root (tests use a temp dir).
 */
...
export function jevBrowserUseHome(): string {
  const env = process.env.JEV_BROWSER_USE_HOME;
  if (env && env.length > 0) return resolvePath(env);
  // Independent state and protocol namespace; never reuse an upstream daemon or profile.
  return joinPath(homedir(), ".jev-browser-use", "v1");
}

export const paths = {
  home: () => jevBrowserUseHome(),
  socket: () => process.env.JEV_BROWSER_USE_SOCKET || joinPath(jevBrowserUseHome(), "daemon.sock"),
  pid: () => joinPath(jevBrowserUseHome(), "daemon.pid"),
  lock: () => joinPath(jevBrowserUseHome(), "daemon.lock"),
  log: () => joinPath(jevBrowserUseHome(), "daemon.log"),
  config: () => joinPath(jevBrowserUseHome(), "config.json"),
  tmp: () => joinPath(jevBrowserUseHome(), "tmp"),
  browsers: () => joinPath(jevBrowserUseHome(), "browsers"),
  /**
  ...
  profile: (name: string, headless = false, insecure = false) =>
    joinPath(jevBrowserUseHome(), "browsers", sanitizeName(name), "profile" + (headless ? "-headless" : "") + (insecure ? "-insecure" : "")),
  /** user-data-dir for `jev-browser-use chrome --profile NAME`; its own root so it never collides with -b NAME. */
  chromeProfile: (name: string) => joinPath(jevBrowserUseHome(), "chrome-profiles", sanitizeName(name)),
  downloads: () => joinPath(jevBrowserUseHome(), "tmp", "downloads"),
  pagesDir: () => joinPath(jevBrowserUseHome(), "pages"),
  pagesFile: (browserKey: string) => joinPath(jevBrowserUseHome(), "pages", sanitizeKey(browserKey) + ".json"),
  chromeDir: () => joinPath(jevBrowserUseHome(), "chrome"),
  chromePorts: () => joinPath(jevBrowserUseHome(), "chrome-ports.json"),
};

/** Ensure the home dir and its standard subdirs exist. Safe to call often. */
export function ensureHome(): void {
  const fs = lazyFs();
  for (const dir of [jevBrowserUseHome(), paths.tmp(), paths.browsers(), paths.pagesDir()]) {
    try {
      ...
      if (code === "EACCES" || code === "EROFS" || code === "EPERM" || code === "ENOTDIR") {
        throw new Error(
          `cannot write to JEV_BROWSER_USE_HOME ${jevBrowserUseHome()} (${code}). Fix its permissions or point JEV_BROWSER_USE_HOME at a writable directory.`,
        );
      }
      throw err;
    }
