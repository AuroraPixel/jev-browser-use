/**
 * Wire protocol between the jev-browser-use client and the daemon.
 *
 ...
 *
 * The same frames are what `jev-browser-use --json` prints, so an MCP server or bb
 * can reuse them without a new contract.
 */

import type { JevConfig } from "../jev/types.ts";
...
/** Launch a Chrome with a persistent named profile under ~/.jev-browser-use/v1/browsers/<name>. */
...
export interface RunRequest {
  ...
  cwd: string;
  /** Caller credentials travel only over the private daemon socket, never in scripts or logs. */
  jev?: JevConfig;
}
