/**
 * User config at ~/.jev-browser-use/v1/config.json and duration parsing.
 *
...
export interface JevBrowserUseConfig {
...
export function loadConfig(): JevBrowserUseConfig {
  try {
    const raw = lazyFs().readFileSync(paths.config(), "utf8");
    const parsed = JSON.parse(raw);
    if (parsed && typeof parsed === "object") return parsed as JevBrowserUseConfig;
  } catch {
...
export async function loadConfigAsync(): Promise<JevBrowserUseConfig> {
  try {
...
export function resolveIdleTimeoutMs(flag: string | undefined, config: JevBrowserUseConfig): number {
  if (flag !== undefined) return parseDuration(flag);
  // Accept the alternate JEV_BROWSER_USE_IDLE_TIMEOUT_MS spelling as a
  // fallback so shell configuration survives the major-version upgrade.
  const env = process.env.JEV_BROWSER_USE_IDLE_TIMEOUT || process.env.JEV_BROWSER_USE_IDLE_TIMEOUT_MS;
