 ...
 *
 *   jev-browser-use [flags] < script.js
 *   jev-browser-use [flags] -e 'code'
 *   jev-browser-use [flags] run FILE
 *   jev-browser-use pages | browsers | status | stop [NAME] | install | install-skill [...] | migrate-from-doobie | chrome [...] | help [topic] | daemon
 */
...
const PASSTHROUGH = new Set(["install", "install-skill", "chrome", "relay"]);
...
export function parseArgs(argv: string[]): ParsedArgs {
        ...
        if (!command) {
          switch (a) {
            ...
            case "mcp":
              ...
              break;
            default:
              throw new UsageError(`unknown command "${a}". Run jev-browser-use --help.`);
          }
        } else {
  ...
  if (command.kind === "install" || command.kind === "install-skill" || command.kind === "chrome" || command.kind === "relay") command.args = rest;
