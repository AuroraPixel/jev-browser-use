...
function acquireSpawnLock(): boolean {
  ...
  try {
    ...
    if (code !== "EEXIST") {
      throw new Error(
        `cannot write to JEV_BROWSER_USE_HOME ${paths.home()} (${code ?? (err as Error).message}). Fix its permissions or point JEV_BROWSER_USE_HOME at a writable directory.`,
      );
    }
    try {
...
export async function ensureDaemon(): Promise<Conn> {
  ...
  throw new Error(
    `jev-browser-use daemon did not start within ${SPAWN_WAIT_MS / 1000}s. Check ${paths.log()} and run \`jev-browser-use status\`.`,
  );
}
...
export interface SendOptions {
  /** Cancelling closes only this request socket, which aborts its daemon run. */
  signal?: AbortSignal;
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      if (err instanceof DaemonHungError && attempt === 0) {
        writeErr("jev-browser-use: daemon unresponsive, restarting it\n");
        killDaemon();
        continue;
      }
      throw err;
...
function talk(conn: Conn, req: Request, opts: SendOptions): Promise<"ok" | "retry"> {
  return new Promise<"ok" | "retry">((resolve, reject) => {
    ...
    const cancel = () => { done = true; reject(new Error("Request cancelled; inspect any in-flight browser action")); conn.destroy(); };
    conn.onData((chunk) => {
      let frames: Frame[];
      try {
