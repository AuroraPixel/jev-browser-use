/**
 * jev-browser-use CLI entry. Keep this module and its static imports small: the
...
import { DEFAULTS, loadConfigAsync, resolveIdleTimeoutMs, formatDuration, type JevBrowserUseConfig } from "../shared/config.ts";
...
export async function main(argv: string[]): Promise<number> {
  ...
  try {
    ...
    if (err instanceof UsageError) {
      writeErr(`jev-browser-use: ${err.message}\n`);
      return EXIT_USAGE;
    }
    throw err;
  }
  ...
  switch (command.kind) {
    ...
    case "help": {
      ...
      if (text === null) {
        writeErr(`jev-browser-use: ${unknownTopicMessage(command.topic)}`);
        return EXIT_USAGE;
      }
      out(text);
      return EXIT_OK;
    ...
    case "relay": {
      const mod = await loadDaemonModule();
      return mod.relayCommand(command.args);
    }
    case "status":
    ...
    case "jev":
      return runScriptCommand(flags, undefined, true);
    case "script":
...
export function sourceFromFlags(flags: GlobalFlags, config: JevBrowserUseConfig): BrowserSourceSpec {
...
async function runScriptCommand(flags: GlobalFlags, file?: string, isJev = false): Promise<number> {
  let src: { script: string; name: string } | null;
  try {
    src = await readScript(flags, file);
    if (src && isJev) {
      let input: unknown;
      try { input = JSON.parse(src.script); } catch { throw new UsageError("jev expects a JSON object on stdin or with -e"); }
      try { src = { script: jevScript(input), name: "<jev>" }; } catch (e) { throw new UsageError((e as Error).message); }
    }
  } catch (err) {
    if (err instanceof UsageError) {
      writeErr(`jev-browser-use: ${err.message}\n`);
  ...
  const timeoutSeconds = flags.timeout ?? config.timeout ?? (isJev ? 60 : DEFAULTS.timeoutSeconds);
  let idleTimeoutMs: number;
  try {
    idleTimeoutMs = resolveIdleTimeoutMs(flags.idleTimeout, config);
  } catch (err) {
    writeErr(`jev-browser-use: ${(err as Error).message}\n`);
    return EXIT_USAGE;
  }
...
export function runCli(): void {
  main(process.argv.slice(2)).then(
    ...
    (e) => {
      writeErr(`jev-browser-use: ${(e as Error)?.stack ?? String(e)}\n`);
      flushAll();
