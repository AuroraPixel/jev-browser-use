 ...
 * file asset and stores its path in globalThis.__JEV_BROWSER_USE_DAEMON_ASSET.
...
export interface DaemonModule {
  ...
  relayCommand(args: string[]): Promise<number>;
  mcpMain(flags: import("./args.ts").GlobalFlags): Promise<number>;
}

declare global {
  // eslint-disable-next-line no-var
  var __JEV_BROWSER_USE_DAEMON_ASSET: string | undefined;
}

export async function loadDaemonModule(): Promise<DaemonModule> {
  const asset = globalThis.__JEV_BROWSER_USE_DAEMON_ASSET;
