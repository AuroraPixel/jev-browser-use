 ...
 * versioned jev-browser-use v1 home. The source is never modified.
 */
...
export function migrateFromDoobie(): number {
  ...
  if (!fs.existsSync(source) || !fs.statSync(source).isDirectory()) {
    process.stderr.write(`jev-browser-use: no doobie state directory found at ${source}\n`);
    return EXIT_ERROR;
  }
  ...
  try {
    ...
    if (processIsAlive(pid)) {
      process.stderr.write(`jev-browser-use: doobie daemon pid ${pid} is still running. Run \`doobie stop\` before migrating.\n`);
      return EXIT_ERROR;
    }
  } catch {
  ...
  if (activePids.length > 0) {
    process.stderr.write(
      `jev-browser-use: doobie Chrome process${activePids.length === 1 ? "" : "es"} ${activePids.join(", ")} ` +
  ...
  if (existing.length > 0) {
    process.stderr.write(
      `jev-browser-use: refusing to overwrite existing v1 state in ${target} (${existing.join(", ")}).\n` +
        "Move that directory aside or use JEV_BROWSER_USE_HOME to select an empty destination.\n",
    );
    return EXIT_ERROR;
  }
  ...
  if (copied.length === 0) {
    process.stderr.write(`jev-browser-use: ${source} contains no durable state to migrate\n`);
    return EXIT_ERROR;
  }
