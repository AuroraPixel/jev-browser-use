/**
 * jev-browser-use chrome [--profile NAME] [--port N] [--chrome PATH] [--headless] [URL]
 *
 * Launch the user's installed Chrome as a normal OS process with remote
 * debugging on a dedicated profile, and remember the port so `jev-browser-use --connect`
...
/**
 * `jev-browser-use chrome` exists so Google/OAuth sign-in works, which rejects
...
export async function chromeCommand(args: string[]): Promise<number> {
  ...
  if (!exe || !picked) {
    process.stderr.write("jev-browser-use chrome: no Chrome found. Pass --chrome /path/to/chrome or run `jev-browser-use install`.\n");
    return EXIT_ERROR;
  }
  if (!chromePath && (picked.source === "installed" || picked.source === "playwright")) {
    process.stderr.write(
      `jev-browser-use chrome: no system Chrome found; using ${picked.source === "installed" ? "Chrome for Testing" : "Playwright's Chromium"} (${exe}).\n` +
  ...
  if (!outcome.ok && !noSandbox && isLinux && isSandboxError(outcome.fullStderr)) {
    process.stderr.write(`jev-browser-use chrome: sandbox unavailable, retrying with --no-sandbox (remembered for ${exe})\n`);
    noSandbox = true;
  ...
  if (!outcome.ok) {
    ...
    let msg = `jev-browser-use chrome: Chrome exited right after launch (${how}).\n  chrome: ${exe}\n`;
  ...
  process.stdout.write(
    `launched Chrome (pid ${outcome.pid}) on port ${chosenPort} with profile ${userDataDir}${outcome.answered ? "" : " (not answering yet; still starting)"}\n` +
      ...
      `use:  jev-browser-use --connect ${chosenPort} -e 'await (await browser.getPage("main")).title()'\n` +
      `or:   jev-browser-use --connect   (auto-discovers this port)\n`,
  );
  return EXIT_OK;
}

function usage(msg: string): number {
  process.stderr.write(`jev-browser-use chrome: ${msg}\n`);
  return EXIT_USAGE;
}
